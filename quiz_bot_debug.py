...
async def handle_private_poll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # DEBUG: Show what we receive from forwarded poll
    debug_info = (
        f"🔍 DEBUG Poll Data:\n"
        f"Question: {poll.question[:50] if poll.question else 'None'}...\n"
        f"Options: {len(poll.options) if poll.options else 0}\n"
        f"Correct ID: {poll.correct_option_id}\n"
        f"Explanation: '{poll.explanation}'\n"
        f"Explanation entities: {poll.explanation_entities}\n"
        f"Type: {poll.type}\n"
        f"Is closed: {poll.is_closed}"
    )
